...
<div class="col-lg-12">
	<div class="row">
		<p>
			<!--<a href='reservation.asp?<%=linkChange("mm", -1)%>'>&lt;</a>-->
			...
			<table class='table table-bordered table-responsive'>
				...
				<%
					...
					Do While iCurrent <= iDIM
						...
						Response.Write vbTab & vbTab & "<td class='" & divClass & "' onClick=""javascript:goReserve('" & spSeq & "', '" & Year(dDate) & "." & fnTwoDigit(Month(dDate)) & "." & fnTwoDigit(iCurrent) & "');"" style='cursor:pointer;'>" & iCurrent
...
<!--
	...
	function goReserve(pa1, pa2){
		...
		if(!pa1){
			...
			var moveUrl = "stayReservationCalendar_1.asp?spSeq=" + pa1 + "&reserveDate=" + pa2;
			$.ajax( {
				url:moveUrl,
	...
	function goMoveDate(pa){
		var moveUrl = "stayReservationCalendar_1.asp?" + pa + "&spSeq=<%=spSeq%>";
		$.ajax( {
			url:moveUrl,
			success:function(data) {
				$('#calendar').html(data);
